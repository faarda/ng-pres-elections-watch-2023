...
const Modal: React.FC<Props> = ({ show, toggle, children }) => {
  return (
    <>
      ...
      >
        <div
          className={`bg-white w-11/12 mx-auto max-w-md rounded-xl pt-8 px-6 sm:pt-10 sm:px-8 transform duration-500 delay-200 origin-bottom transition-all ${
            show ? "scale-100 opacity-100 translate-y-0" : "scale-90 opacity-0 translate-y-10"
          }`}
        >
          <button
            className="absolute top-3.5 right-3.5 h-6 w-6 flex items-center justify-center transition-colors duration-150 text-grey-text hover:text-black no-outline"
